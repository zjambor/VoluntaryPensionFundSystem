...
{
    public partial class OkiratokModositas : VoluntaryPensionFundSystem.BaseForm
    {
        ...
        TextWriterTraceListener myListener = new TextWriterTraceListener(@"LOG\OkiratokModositasa.log", "myListener");
        public const string MatchEmailPattern = @"^(([\w-]+\.)+[\w-]+|([a-zA-Z]{1}|[\w-]{2,}))@" + @"(([a-zA-Z]+[\w-]+\.)+[a-zA-Z]{2,4})$";
        public const string MatchAdoszamPattern = @"^[0-9]{10}$";

        public OkiratokModositas(SqlConnection SqlConn)
        {
            InitializeComponent();
        ...
        private void dgwOkiratok_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cbPortfolio.Text = dgwOkiratok.Rows[i].Cells[39].Value.ToString();
                ...
                if (dgwOkiratok.RowCount > 0)
                {
                    bMunkaviszony.Enabled = true;
                    bErvenytelen.Enabled = true;
                    bKedvezm.Enabled = true;
                }
            }
            catch
            {
        ...
        public override void save()
        {
            ...
            //for (int k = 0; k < dgwOkiratok.RowCount; k++)
            //{
            //    if ((int)dgwOkiratok.Rows[k].Cells[0].Value == selectedId)
            //    {
            //        dgwOkiratok.CurrentCell = dgwOkiratok.Rows[k].Cells[1];
            //        break;
            //    }
            //}
        ...
        public override void yellowMode()
        {
            ...
            bMunkaviszony.Enabled = false;
            bErvenytelen.Enabled = false;
            bKedvezm.Enabled = false;
        ...
        public override void runQuery()
        {
            ...
            else
            {
                ...
                querystring = "SELECT pnr.pnr_id,tszs.tszs_id,pnr.nev,pnr.adoazonosito_jel," +
                    ...
                    "tszs.felvet_datum,tszs.zaradek_datum,tszs.eredeti_tagsag,tszs.szamla_lezar,tszs.portfolio " +
                ...
                querystring += "(pnr.ervenytelen is null or pnr.ervenytelen!='I') order by pnr.nev;";
        ...
        private void bErvenytelen_Click(object sender, EventArgs e)
        {
            if (!(txPnrid.Text == string.Empty))
            {
                DialogResult dr = MessageBox.Show("Biztosan érvényteleníteni akarja a kijelölt rekordot?\nPnr Id: " + txPnrid.Text, "Megerősítés", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                if (dr == DialogResult.OK)
                {
                    txNev.Text = "/" + txNev.Text;
                    string query = "update partnerek set ervenytelen='I',nev='" + txNev.Text + "' where pnr_id=" + txPnrid.Text;
                    scommand = new SqlCommand(query, sconn);
                    scommand.ExecuteNonQuery();
                    
                    int i = dgwOkiratok.SelectedCells[0].RowIndex;
                    dgwOkiratok.Rows.RemoveAt(i);
                    MessageBox.Show("Az okirat érvénytelenítése sikeres. Az adatok a Partner Törzsadatok felületen módosíthatóak.");
                }
            }
        }

        private void NevKeres()
        {
            txNev.ReadOnly = true;
            //txNev.BackColor = System.Drawing.SystemColors.Control;
            ...
            for (int k = 0; k < dgwOkiratok.RowCount; k++)
            {
                try
                {
                    if (dgwOkiratok.Rows[k].Cells[2].Value.ToString().Substring(0, txNev.Text.Length) == txNev.Text)
                    {
                        //x = (int)dgwOkiratok.Rows[k].Cells[1].Value;
                        dgwOkiratok.CurrentCell = dgwOkiratok.Rows[k].Cells[2];
                        talal = true;
                        keresomod = false;
                        break;
                    }
            ...
            if (!talal)
            {
                MessageBox.Show("Nincs találat!", "Megerősítés", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txNev.ReadOnly = false;
                txNev.BackColor = System.Drawing.Color.Yellow;
                txNev.Focus();
            }
        }

        private void OkiratokModositas_Load(object sender, EventArgs e)
        {
        ...
        private void bMunkaviszony_Click(object sender, EventArgs e)
        {
            Fogl_munkaviszony LabourRelation = new Fogl_munkaviszony(sconn, int.Parse(txTszsId.Text));
            LabourRelation.Show();
        }

        private void bKedvezm_Click(object sender, EventArgs e)
        {
            Kedvezmenyezettek kedv = new Kedvezmenyezettek(sconn, int.Parse(txPnrid.Text), this);
            kedv.Show();
        }
